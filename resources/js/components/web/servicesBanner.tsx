        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className="group shadow-teal-100 rounded-t-xs bg-white p-6 border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.02] hover:border-gray-300"
              >
                {/* Icon + Title inline */}
                <div className="flex items-start gap-4 mb-4">
                  <div
                    className={`flex items-center justify-center w-14 h-14 bg-gradient-to-r ${service.color} shadow-sm group-hover:shadow-md transition-shadow duration-300`}
                  >
                    <IconComponent className="h-7 w-7 text-white" />
                  </div>
                  <h3 className={`text-base font-bold leading-snug flex-1 ${service.text}`}>
                    {service.title}
                  </h3>
                </div>

                {/* Description */}
                <p className="text-black text-sm font-normal leading-relaxed border-t border-gray-100 pt-4 group-hover:border-gray-200 transition-colors duration-300">
                  {service.description}
                </p>
              </div>
            );
