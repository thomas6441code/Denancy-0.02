...
const TopSection = ({
  images = [
    {
      ...
      description:
        'We are your trusted partner for innovative solutions and exceptional service delivery in Tanzania and beyond.',
    },
  ],
  ...
  /**
   * ✅ Only store images once — ignore prop changes after mount
   * This prevents Inertia re-renders from resetting the slideshow
   */
  const initialImagesRef = useRef<ImageItem[]>(images);
  const [currentIndex, setCurrentIndex] = useState(0);
  const intervalRef = useRef<NodeJS.Timeout | null>(null);

  /** ✅ Stable image reference */
  const stableImages = initialImagesRef.current;
  ...
  const backgroundImage = stableImages[currentIndex]?.url || '';
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden flex items-center justify-center">
        <div className="relative w-full h-full">
          <div
            ...
            aria-label={stableImages[currentIndex]?.title || 'Background image'}
          />
        </div>
      </div>
        ...
        <div className="container mt-5 mx-auto px-4 text-white">
          <div className="relative overflow-hidden">
            ...
            <div key={`title-${currentIndex}`} className="overflow-hidden">
              <h1
                className="text-3xl md:text-5xl font-bold leading-tight md:leading-snug max-w-4xl ml-0 md:ml-10 text-left
                  ...
                  cursor-pointer p-2"
              >
                {stableImages[currentIndex]?.title}
              </h1>
            </div>
            ...
            <div key={`description-${currentIndex}`} className="overflow-hidden">
              <p
                className="mt-4 text-sm md:text-lg max-w-3xl ml-0 md:ml-10 text-left
                  ...
                  cursor-pointer p-2"
              >
                {stableImages[currentIndex]?.description}
              </p>
            </div>
          </div>
            ...
            {showBanner != 'home' && showBanner != 'contact' && (
              ...
              >
                ...
                >
                  Call Now{' '}
                  <PhoneIncoming
                    className={`ml-2 p-1  rounded-xs text-white bg-${secondaryButtonColor}-400 h-5 w-5`}
                  />
                </a>
              </div>
            )}
